...
describe('<FleetLoadInMWH />', () => {
  const mountComponent = (props = {}) =>
    mount(
      <IntlProvider locale="en">
        <FleetLoadInMWH {...props} />
      </IntlProvider>
    );

  describe('#constructor', () => {
    it('should render < FleetLoadInMWH /> component when no chart data is sent', () => {
      const expected = true;
      const fleetLoadInMWHWrapper = mountComponent({
        config: reportConfigMock,
      });

      const actual = fleetLoadInMWHWrapper.exists();
      expect(actual).toBe(expected);
    });

    it('should render < FleetLoadInMWH /> component when chart data is sent', () => {
      const expected = true;
      const fleetLoadInMWHWrapper = mountComponent({
  ...
  describe('#OutRangeAlert', () => {
    it('should not display OutRangeAlert when scale is auto', () => {
      ...
      const actual = fleetLoadInMWHWrapper
